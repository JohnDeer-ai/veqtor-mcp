...
from .contracts import (
    ...
    normalized_internal_package_part_name as _normalized_internal_package_part_name,
)
...
def require_single_direct_document_body(
    ...
    revision_tags = TEXT_REVISION_TAGS | MOVE_REVISION_TAGS | UNSUPPORTED_REVISION_TAGS
    for child in document:
        if child is body:
            continue
        if any(element.tag in revision_tags for element in child.iter()):
            raise DocxError(
                "word/document.xml revision markup must be inside the direct w:body"
            )
    return body
...
def normalized_internal_package_part_name(value: object) -> str | None:
    """Compatibility alias for the shared live/history package-part domain."""
    return _normalized_internal_package_part_name(value)
