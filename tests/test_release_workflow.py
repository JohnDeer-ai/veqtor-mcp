...
@pytest.mark.parametrize("document_name", ["README.md", "CONTRIBUTING.md"])
def test_public_checklists_separate_development_from_frozen_release_commands(
    document_name: str,
) -> None:
    document = (ROOT / document_name).read_text()
    development_marker = "### Development checks\n"
    frozen_marker = "### Frozen-release checks\n"
    assert document.count(development_marker) == 1
    assert document.count(frozen_marker) == 1
    development = document.split(development_marker, 1)[1].split(frozen_marker, 1)[0]
    frozen = document.split(frozen_marker, 1)[1]
    frozen_commands = (
        "scripts/build_mcpb.py",
        "scripts/check_release_artifacts.py",
        "scripts/check_mcpb_artifact.py",
        "@anthropic-ai/mcpb@2.1.2 validate",
    )

    assert "uv build --clear" in development
    assert "uvx twine check dist/*.whl dist/*.tar.gz" in development
    assert all(command not in development for command in frozen_commands)
    assert all(command in frozen for command in frozen_commands)
    assert "[project].version" in frozen
    assert "scripts/release_contract.py::VERSION" in frozen
...
def test_development_wheel_smoke_binds_metadata_and_nine_tool_v04_surface() -> None:
    ...
    assert "from veqtor_mcp._inspection_live import CheckedInspectionResult" in smoke
    assert (
        'CheckedInspectionResult.__module__ == "veqtor_mcp._inspection_live"' in smoke
    )
    assert '"inspect_document"' in smoke
    assert 'session.call_tool(\n                    "inspect_document"' in smoke
    assert 'session.call_tool(\n                    "map_rounds"' in smoke
    assert '"trace_paragraph_history"' in smoke
    assert '"pending_text_revisions_rejected_v1"' in smoke
    assert '"verification_result.v2"' in smoke
    assert '"paragraph_history.v1"' in smoke
    assert 'inspected["revision_inventory"]["schema_version"]' in smoke
    assert "def _assert_producer(payload: dict)" in smoke
    assert smoke.count("_assert_producer(") == 15
