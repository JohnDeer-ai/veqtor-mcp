...
def test_extract_and_inspect_reject_revision_outside_direct_body(
    demo_dir: Path,
    tmp_path: Path,
) -> None:
    source = demo_dir / "round-1-outgoing-draft.docx"
    invalid = tmp_path / "revision-outside-body.docx"

    def add_revision_sibling(document: etree._Element) -> None:
        body = document.find(w("body"))
        assert body is not None
        revision = etree.Element(w("ins"))
        revision.set(w("id"), "outside-body-1")
        revision.set(w("author"), "Inventory Test")
        _run(revision, "Revision outside body")
        document.insert(document.index(body), revision)

    _rewrite_document(source, invalid, add_revision_sibling)
    expected_sha256 = hashlib.sha256(invalid.read_bytes()).hexdigest()

    for operation in (
        lambda: extract_redlines(str(invalid)),
        lambda: inspect_document(str(invalid), "browse"),
    ):
        with pytest.raises(DocxError) as error:
            operation()
        assert "revision markup must be inside the direct w:body" in str(error.value)
        assert error.value.metadata["observed_source_sha256"] == expected_sha256
