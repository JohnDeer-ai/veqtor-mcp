...
`veqtor.mcp.v0.3`. The Stage 3A code, package, CI and Claude Desktop acceptance
were completed for exact source commit `d2fc8fb30819581e4b795b49c5f7831dff3a81fa`.
That evidence does not accept later contract-polish commits and does not claim
that `0.3.0` or any corresponding artifact has been publicly released.
...
Recognized revision markup outside that direct body is likewise refused and
never classified as decoded.
...
The complete `section_ref.v1` object is the section identity. In particular,
`heading_text_sha256` alone is not unique: identical heading text may occur at
several canonical positions in the same document.
...
the only occurrence. A snippet is navigation output even when both truncation
flags are false: only a complete `read` result followed by `verify_quote`
supports a quotation claim under this contract.
...
The two outer indexed counters cover every paragraph in canonical body flow,
including table-cell paragraphs. The nested `container_coverage` then
partitions that same total into `body_paragraph_count` and
`table_cell_paragraph_count`; the word "body" there names the direct body
container rather than the complete indexed main-body scope.
...
## Successful-result runtime boundary

The authoritative producer builds the public result dictionary directly from
one parsed DOCX snapshot. The advertised JSON Schema requires the stable common
and mode-specific core while keeping only the outer result additive. Nested
coverage, limits, inventories, references, navigation and item/snippet objects
are closed, apart from documented typed counter maps. After serialization, the
transport-neutral runtime gate applies only the bounded consistency and safety
checks listed below before a success may be returned or journaled.

This gate does not independently prove producer origin, page membership,
ordering, paragraph or section hashes, literal attribution, or cursor binding
to the underlying DOCX. Those producer responsibilities are covered by focused
fixture, property and metamorphic tests. The former envelope/canonical-reference
guarantees are retired rather than represented as independent runtime proof.
Historical `inspection.v1` projection remains a separate,
compatibility-tolerant path and does not run the live-result gate.

| Check ID | Retained consistency or safety relationship |
| --- | --- |
| `constants.policy-scope-limits-v1` | Policy identifiers, scan status, included scope and fixed v0.3 limits equal their contract constants; request and indexed counts stay within their disclosed bounds. |
| `payload.collection-count-v1` | `coverage.returned_item_count` equals the length of the mode collection. |
| `page.mode-coverage-v1` | Browse, outline and read eligibility/count relationships remain internally consistent. |
| `page.pagination-v1` | Page size, offset, truncation, cursor presence and next-offset relationships are internally consistent. |
| `page.literal-binding-v1` | Literal complete count, phrase index, match basis and occurrence bounds are internally consistent. |
| `snapshot.indexed-container-v1` | Serialized outer and container paragraph counters satisfy their stated equations. |
| `snapshot.container-exclusions-v1` | Serialized exclusion counters, maps and completeness flags satisfy their stated equations. |
| `snapshot.revision-partitions-v1` | Serialized revision partitions, maps, kind counts and duplicated container summary agree. |
| `snapshot.revision-flags-v1` | Serialized revision completeness flags agree with unsupported and excluded occurrence counts. |
| `snapshot.revision-global-v1` | `decoded <= tracked <= total`; decoded content requires the top-level warning, and a true warning requires a non-zero total. |
| `payload.returned-text-caps-v1` | Returned `text`/`heading`/`label` values remain within the individual and aggregate response caps. |
| `snapshot.excluded-scope-v1` | `excluded_parts` retains the fixed public prefix and a normalized, unique, sorted dynamic tail. |
| `payload.returned-revision-warning-v1` | A returned item flagged with tracked text revisions requires the top-level warning. |

A violation is `output_contract_error`. The diagnostic path used internally to
locate the failed check is sanitized from both the MCP response and the error
decision record.
...
The outer object is forward-compatible and therefore has
`additionalProperties: true`. Its required common core is `mode`, `path`,
`file_sha256`, `part_name`, `search_scope`, `reading_mode`, `container_policy`,
`has_tracked_text_revisions`, `revision_inventory`, `coverage`, `limits`,
`next_cursor`, `producer`, `record_id`, and `record_status`; `record_error` is
required only by the `write_failed` metadata tuple. A successful producer may
not occupy the reserved `status`, `error_code`, `error`, producer or record
metadata fields.

The known mode-owned fields are closed even though unrelated future top-level
extensions remain allowed. `outline` requires only `sections`;
`literal_search` requires `matches`, `match_basis`, and `phrase_count`; `browse`
requires `paragraphs`; paragraph `read` requires `paragraphs` and
`selection_kind`; section `read` additionally requires `section_navigation`.
The schema rejects every known field owned by another mode. Nested producer,
coverage, limits, revision/container inventory, reference, navigation,
paragraph, section, match and snippet objects use `additionalProperties: false`;
only the documented typed counter maps accept dynamic keys.

The dedicated live journal sink accepts only the immutable result created after
schema and bounded runtime checks. Its private raw JSONL record stores a
minimized result/provenance summary and `tool_result_sha256` over the exact
metadata-free checked payload. That preimage includes `producer`, excludes
record metadata, and has no synthetic `status: "ok"` member. The journal does
not store the full paragraph, section or match collection. Compact export
additionally omits paths, digests the closed input, and retains source hash,
mode, policy identifiers, coverage/limit counters, producer identity and a
bounded digest/sample of observed references. The private raw journal still
retains the canonical workspace, request paths and literal-search phrases and
must remain private. This remains a best-effort, local, non-tamper-evident
assurance boundary.
...
The exact Stage 3A snapshot named in the status section passed this automated
fixture set and the corresponding Claude Desktop product acceptance. Any later
polish snapshot must repeat its applicable gates. Product acceptance remains
separate from MCPB installation and public release acceptance.
